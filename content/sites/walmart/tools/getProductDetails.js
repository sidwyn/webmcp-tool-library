...
const GetProductDetailsTool = {
  ...
  description: 'Get detailed information about a product on Walmart. If on a product detail page, reads the current product. If given a rank number from search results, navigates to that product.',
  inputSchema: {
    type: 'object',
    properties: {
      rank: {
        type: 'integer',
        description: 'The rank number of the product from search results to navigate to. Omit if already on a product detail page.'
      }
    }
  },
  ...
  execute: async (args) => {
    ...
    if (rank) {
      if (!url.includes('walmart.com/search') && !url.includes('walmart.com/browse') && !url.includes('walmart.com/cp/')) {
        return { content: [{ type: 'text', text: 'ERROR: rank can only be used on a search results or category page.' }] };
      }

      const cards = WebMCPHelpers.findWalmartProductCards();
      if (rank < 1 || rank > cards.length) {
        return { content: [{ type: 'text', text: `ERROR: rank ${rank} is out of range. There are ${cards.length} results.` }] };
      }
    ...
    if (!url.includes('walmart.com/ip/')) {
      return { content: [{ type: 'text', text: 'ERROR: Not on a Walmart product detail page. Provide a rank number to navigate from search results.' }] };
    }
