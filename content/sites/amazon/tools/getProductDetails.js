...
const GetProductDetailsTool = {
  ...
  execute: async (args) => {
    ...
    if (rank) {
      ...
      const link = (h2 && h2.closest('a')) || card.querySelector('h2 a') || card.querySelector('a.a-link-normal[href*="/dp/"]');
      if (link) {
    ...
    if (availEl) {
      ...
      availability = availSpan ? availSpan.textContent.trim() : availEl.textContent.trim().replace(/\s+/g, ' ').replace(/\{.*\}/, '').trim();
    }
    if (!availability) {
      const outOfStock = document.querySelector('#outOfStock');
      if (outOfStock) availability = 'Out of Stock';
    }
