...
const SortResultsTool = {
  name: 'sort_results',
  description: 'Change the sort order of Amazon search results.',
  inputSchema: {
    type: 'object',
    properties: {
      sortBy: {
  ...
  execute: async (args) => {
    ...
    const sortLabels = {
      featured: 'Featured', price_low_to_high: 'Price: Low to High',
      price_high_to_low: 'Price: High to Low', avg_customer_review: 'Avg. Customer Review',
    ...
    // Fallback: URL-based sort
    const sortMap = {
      featured: '', price_low_to_high: 'price-asc-rank', price_high_to_low: 'price-desc-rank',
      avg_customer_review: 'review-rank', newest_arrivals: 'date-desc-rank'
    };

    const url = new URL(window.location.href);
    const sortVal = sortMap[sortBy];
    if (sortVal) url.searchParams.set('s', sortVal);
