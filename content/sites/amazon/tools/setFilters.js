...
const SetFiltersTool = {
  name: 'set_filters',
  description: 'Apply filters to Amazon search results. Supports price range, Prime-only, minimum star rating, brand, and condition.',
  inputSchema: {
    type: 'object',
    properties: {
      minPrice: {
  ...
  execute: async (args) => {
    ...
    if (args.minRating) {
      const ratingLabels = {
        4: '4 Stars & Up', 3: '3 Stars & Up', 2: '2 Stars & Up', 1: '1 Star & Up'
      };
    ...
    // ── Condition ───────────────────────────────────────────────────────────
    if (args.condition) {
      const conditionLabels = { new: 'New', used: 'Used', renewed: 'Renewed' };
    ...
    if (actions.length === 0) {
      return {
        content: [{ type: 'text', text: 'No filters specified. Available: minPrice, maxPrice, primeOnly, minRating, brand, condition.' }]
      };
    }

    return {
      content: [{ type: 'text', text: `Filters applied:\n${actions.join('\n')}\n\nCall get_results to see the updated products.` }]
    };
  }
};
