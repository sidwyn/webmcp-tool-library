...
const BuyNowTool = {
  name: 'buy_now',
  description: 'Click "Buy Now" on the current Amazon product page to proceed to checkout. Stops at the checkout summary — does NOT place the order.',
  inputSchema: {
    type: 'object',
    properties: {
      quantity: {
      ...
      variant: {
        type: 'object',
        description: 'Variant options to select before buying (e.g., {"Size": "Large", "Color": "Blue"}).'
      }
    }
  },
  ...
  execute: async (args) => {
    ...
    if (isCheckout) {
      return {
        content: [{
          type: 'text',
          text: 'Arrived at checkout page. Call get_checkout_summary to review the order details before confirming. IMPORTANT: Do NOT place the order without explicit user confirmation.'
        }]
      };
    }
