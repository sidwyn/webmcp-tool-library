...
const SearchProductsTool = {
  ...
  execute: async (args) => {
    ...
    const categoryMap = {
      all: '', electronics: 'electronics', books: 'stripbooks', clothing: 'fashion',
      home: 'garden', toys: 'toys-and-games', sports: 'sporting', beauty: 'beauty',
      grocery: 'grocery', automotive: 'automotive', garden: 'lawngarden', health: 'hpc',
      'pet-supplies': 'pets', office: 'office-products', tools: 'tools'
    };

    const sortMap = {
      relevance: '', price_low_to_high: 'price-asc-rank', price_high_to_low: 'price-desc-rank',
      avg_customer_review: 'review-rank', newest_arrivals: 'date-desc-rank'
    };

    let url = `https://www.amazon.com/s?k=${encodeURIComponent(query.trim())}`;
    const catVal = categoryMap[category];
    if (catVal) url += `&i=${catVal}`;
