...
INTENT MAPPING:
- "buy/order/purchase X" → search_products → get_results → user picks → add_to_cart or buy_now
- "find/search/look for X" → search_products → get_results
- "compare X" → search_products → get_results → compare_products
- "reviews for X" → navigate to product → get_reviews
- "cheapest X" → search_products with sortBy: price_low_to_high → get_results
- "best rated X" → search_products with sortBy: avg_customer_review → get_results
- "Prime only X" → search_products → set_filters with primeOnly: true
- "under $X" → search_products → set_filters with maxPrice
- "what's in my cart" → get_cart
- "deals/coupons" → navigate to product → check_price_history
