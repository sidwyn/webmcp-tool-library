...
globalThis.window = { location: { href: 'https://www.target.com/s?searchTerm=headphones' } };
globalThis.document = {
  querySelector: () => null,
  querySelectorAll: () => [],
  dispatchEvent: () => {},
...
globalThis.WebMCPHelpers = {
  ...
  waitForTargetResults: () => Promise.resolve(true),
  waitForTargetPDP: () => Promise.resolve(true),
  parseTargetProductCard: () => ({}),
  simulateTargetSearch: () => Promise.resolve(true),
};

const toolDir = join(__dirname, '../content/sites/target/tools');
...
describe('Target tool schemas', () => {
  const toolEntries = Object.entries(tools);

  it('loaded at least 9 tools', () => {
    expect(toolEntries.length).toBeGreaterThanOrEqual(9);
  });
